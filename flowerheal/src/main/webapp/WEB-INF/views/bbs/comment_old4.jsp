...
<script>
	...
	rereply_str +=' <div data-form="write">';
	rereply_str +='    <div class="row mt-2">';
    rereply_str +='        <div class="col-2"></div>';
    rereply_str +='        <div class="col">';
    rereply_str +='           <textarea class="form-control" id="recommentContent" name="rcontent" rows="2"';
    ...
    // 대댓글 수정 html 태그 끝 -----------------------------------
    var replyModify_str = '';
    replyModify_str += '   <div data-form="modify">';
    replyModify_str += '   <div class="row mt-2">';
    replyModify_str += '        <div class="col-2"></div>';
				...
				success:function(result){		
					$.each(result.item,function(idx, rec){
						...
						if(rec.rindent == 0){
							...
							str += '<div class="row justify-content-end p-0">';
							str += '  <div class="col-1">';
/* 							str += '    <img src="${pageContext.request.contextPath }/resources/img/login.jpg"';
						...
						if(l_id == rec.rid){
							...
							str += '    <a class="badge badge-pill badge-warning">수정</a>';
							str += '    <a class="replyDelBtn badge badge-pill badge-danger">삭제</a>';
							str += '  </div>';	
						}
						str += '</div>';	
						str += '  </div>';			// <div data-rnum> 닫는 태그	
					});
		...
		function doActionEvent(){
			...
			$(".rereplyBtn").on("click",function(){
				...
				// 대댓글 양식이 없는 경우에만 추가될수있도록 구분
				if($data_rnum != $data_rrnum){
					// 요소1.after(html 태그 요소2) : 요소1. 다음줄에 태그2 요소를추가한다. 
					
					
				
				// 현재글의 대댓글은 있으나 댓글 작성 중이 아닐 경우
				} else if($data_rnum == $data_rrnum && data_form == null) {
					alert("data_form ==null");
					
					$divEle.after(rereply_str);
					
				// 현재글의 대댓글이 있고 댓글 작성모드일 경우
					...
					$divEle.next().replaceWidth(rereply_str);
					$divEle.next().find("textarea").val($tmp);
					
					
				// 현글의 대댓글이 있고 댓글 수정모드일 경우
